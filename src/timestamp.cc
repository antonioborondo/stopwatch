...
Timestamp::Timestamp(const std::string& timestamp) : timestamp_{timestamp} {
  if (!IsValid()) {
    throw std::invalid_argument("Invalid timestamp");
  }
}

std::string Timestamp::Get() const { return timestamp_; }
...
Timestamp Timestamp::GetCurrent() {
  ...
  if (std::strftime(timestamp, timestamp_size, "%F %T", localtime(&time)) ==
      0) {
