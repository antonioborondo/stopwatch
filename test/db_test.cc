...
class DbTest : public testing::Test {
 protected:
  ...
  void TearDown() override { db_.DeleteRecords(); }
};
...
TEST_F(DbTest,
       AddNewRecordSucceedsWhenLastOneIsFromSameTypeButFromDifferentDay) {
  const Record record_1{Type::kStart, Timestamp("0001-01-01 01:01:01")};
  ASSERT_TRUE(db_.AddRecord(record_1));
...
TEST_F(DbTest, GetSummaryWithPreviousDays) {
  ASSERT_TRUE(
      db_.AddRecord(Record{Type::kStart, Timestamp("2023-10-22 09:00:00")}));
  ASSERT_TRUE(
      db_.AddRecord(Record{Type::kStop, Timestamp("2023-10-22 10:00:00")}));
  ASSERT_TRUE(
      db_.AddRecord(Record{Type::kStart, Timestamp("2023-10-23 09:00:00")}));
  ASSERT_TRUE(
      db_.AddRecord(Record{Type::kStop, Timestamp("2023-10-23 10:00:01")}));
  ASSERT_TRUE(
      db_.AddRecord(Record{Type::kStart, Timestamp("2023-10-24 09:00:00")}));
  ASSERT_TRUE(
      db_.AddRecord(Record{Type::kStop, Timestamp("2023-10-24 10:00:00")}));
...
TEST_F(DbTest, GetRecords) {
  ...
  ASSERT_THAT(records,
              testing::ElementsAre(record_0, record_1, record_2, record_3));
}
